			...
			while (!exit) {
				...
				try {
					...
					case 5: // close a/c
						...
						break;
					case 6: // display based upon value based criteria
					...
					case 7: // remove loan accounts
						Iterator<BankAccount> itr = accounts.values().iterator();
						System.out.println("removing loan accounts");
//						for(BankAccount a : collection) //implicit itr : raises ConcurrentModificationExc
//							if(a.getAcType()==LOAN)
//							collection.remove(a);
						while (itr.hasNext()) {
						...
						// values/keySet/entrySet , Colleciton is backed by the Map n vice versa.
